...
## Domain Redirect to weboryskills.in

The project is configured to automatically redirect all traffic to **weboryskills.in**.

### Configuration

A `vercel.json` file has been added with redirect rules:

- All non-weboryskills.in domains redirect to weboryskills.in
- Preserves the URL path (e.g., `/courses` → `https://weboryskills.in/courses`)
...
- Add `weboryskills.in`
- Optionally add `www.weboryskills.in`
...
Set `NEXTAUTH_URL` to `https://weboryskills.in`

**4. Test:**
After DNS propagates (up to 48 hours):

- `weboryskills.in` → Works directly
- `webory-skill.vercel.app` → Redirects to `weboryskills.in`

...
export default function CodeViz() {
  ...
  return (
    <div className={`flex flex-col h-screen ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-[#fcfcfc] text-gray-900'} font-sans overflow-hidden select-none transition-all duration-500`}>
      {/* Responsive Navbar */}
         ...
         <div className="flex items-center gap-4 lg:gap-10">
            <div className="flex items-center gap-2 lg:gap-3 group cursor-pointer">
               ...
               <div className="flex flex-col leading-none">
                  ...
                  <span className="text-[8px] lg:text-[9px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">by webory skills</span>
               </div>
            </div>
            ...
            <div className="flex items-center gap-2 lg:gap-6">
                {!isMobile && (
                  <div className="relative">
                    <button onClick={() => setIsGalleryOpen(!isGalleryOpen)} className="flex items-center gap-2 px-3 lg:px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/5 group">
                        <BookOpen className="w-3.5 h-3.5 text-gray-500 group-hover:text-white" />
                        <span className="hidden sm:inline text-[10px] lg:text-[11px] font-black uppercase tracking-widest text-gray-400 group-hover:text-white">Learn Logic</span>
                        <ChevronDown className="w-3 h-3 text-gray-600" />
                    </button>
                    <AnimatePresence>
                      {isGalleryOpen && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-12 left-0 w-[240px] bg-black/95 backdrop-blur-3xl border border-white/10 rounded-2xl p-4 shadow-2xl z-50">
      ...
      <main className={`flex-1 flex ${isMobile ? 'flex-col overflow-y-auto' : 'flex-row overflow-hidden'} relative`}>
        ...
        {/* Floating Playback Controls - Adaptive */}
        {activeTab !== 'code' && (
          <div className={`fixed ${isMobile ? 'bottom-6 left-1/2 -translate-x-1/2 w-[90%]' : 'bottom-10 right-10'} px-6 py-4 border rounded-[36px] flex items-center justify-between sm:justify-center gap-4 lg:gap-10 shadow-2xl z-[60] backdrop-blur-[60px] ${theme === 'dark' ? 'bg-black/80 border-white/10' : 'bg-white/95 border-gray-200 shadow-xl shadow-black/5 text-gray-900'}`}>
            <div className="flex items-center gap-6 lg:gap-8">
              <button onClick={() => setCurrentStep(prev => Math.max(0, prev - 1))} className="text-gray-600 hover:text-indigo-400 transition-all transform hover:scale-125"><ChevronLeft className="w-6 h-6" /></button>
              <button 
                  onClick={() => setIsRunning(!isRunning)} 
                  className="w-12 h-12 lg:w-14 lg:h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-75 text-white"
                  style={{ backgroundColor: accentColor.hex }}
              >
                  {isRunning ? <Pause className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 fill-current ml-1" />}
              </button>
              <button onClick={() => setCurrentStep(prev => Math.min(steps.length - 1, prev + 1))} className="text-gray-600 hover:text-indigo-400 transition-all transform hover:scale-125"><ChevronRight className="w-6 h-6" /></button>
            </div>
            
            <div className={`flex items-center gap-4 ${isMobile ? 'hidden sm:flex' : 'pl-8 border-l border-white/10'}`}>
                <div className="flex items-center gap-3 bg-black/20 px-3 py-1.5 rounded-2xl">
                  <button onClick={() => setZoom(prev => Math.max(0.5, prev - 0.1))} className="text-gray-600 hover:text-white"><ChevronLeft className="w-3 h-3" /></button>
                  <span className="text-[9px] font-black tabular-nums">{Math.round(zoom * 100)}%</span>
                  <button onClick={() => setZoom(prev => Math.min(2, prev + 0.1))} className="text-gray-600 hover:text-white"><ChevronRight className="w-3 h-3" /></button>
                </div>
                <button onClick={() => { setCurrentStep(0); setIsRunning(false); }} className="text-gray-600 hover:text-white transition-all transform hover:rotate-[-180deg] duration-700"><RotateCcw className="w-5 h-5" /></button>
            </div>
            
            {/* Reset button for mobile */}
            <button onClick={() => { setCurrentStep(0); setIsRunning(false); }} className={`p-3 rounded-full bg-white/5 border border-white/5 text-gray-600 sm:hidden`}>

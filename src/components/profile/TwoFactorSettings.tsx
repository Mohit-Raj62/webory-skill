  ...
  const handleEnable2FA = async () => {
    ...
    try {
      const res = await fetch("/api/auth/2fa/enable", {
        ...
        body: JSON.stringify({ token: verificationCode, setupSecret }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!twoFactorEnabled && !showSetup && (
        <div className="p-6 border border-white/5 bg-slate-900/50 rounded-xl space-y-4">
          <p className="text-slate-300 text-sm">Protect your account by requiring a code from your authenticator app every time you log in.</p>
      ...
      {twoFactorEnabled && recoveryCodes.length === 0 && (
        <div className="p-6 border border-red-500/20 bg-slate-900/50 rounded-xl space-y-4">
          ...
          <p className="text-sm text-slate-400">To disable 2FA, please enter a code from your authenticator app.</p>
          <div className="flex items-center gap-3">
            <Input 
              value={verificationCode}
              onChange={(e) => setVerificationCode(e.target.value.replace(/\D/g, '').slice(0, 6))}

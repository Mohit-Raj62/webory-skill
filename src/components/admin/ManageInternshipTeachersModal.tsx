...
interface ManageInternshipTeachersModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function ManageInternshipTeachersModal({
  isOpen,
  onClose,
  internship,
  onSuccess,
}: ManageInternshipTeachersModalProps) {
  ...
  const primaryInstructorId = internship?.instructor;

  useEffect(() => {
    if (isOpen && internship) {
      fetchTeachers();
      setSelectedTeacherIds(internship.coInstructors || []);
      setSearchTerm("");
    }
  }, [isOpen, internship]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/internships/${internship._id}`, {
        method: "PUT",
      ...
      <DialogContent className="sm:max-w-[550px] bg-[#111] border-white/10 text-white">
        <DialogHeader>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Assign or remove teachers for <span className="text-white font-semibold">{internship.title}</span>.
          </p>
        </DialogHeader>

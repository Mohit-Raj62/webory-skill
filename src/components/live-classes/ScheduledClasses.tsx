...
export default function ScheduledClasses({ role }: ScheduledClassesProps) {
    ...
    const fetchCourses = async () => {
        try {
            const res = await fetch(role === "admin" ? "/api/admin/courses" : "/api/teacher/courses");
            if (res.ok) {
                const data = await res.json();
                setCourses(role === "admin" ? data.courses : data.data || []);
            }
        } catch (error) {
    ...
    return (
        <div className="mt-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-2xl font-bold text-white mb-2">Scheduled Live Classes</h2>
                    <p className="text-gray-400">Manage your scheduled video sessions</p>
                </div>
                <Button 
                    onClick={openModal}
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {liveClasses.map((liveClass) => (
                        <div key={liveClass._id} className="bg-slate-900/40 border border-white/5 p-6 rounded-2xl relative group">
                            ...
                            <div className="flex flex-col gap-2">
                                {liveClass.meetingUrl ? (
                                    ...
                                    <Link 
                                        href={`/${role}/live-classes/room/${liveClass._id}`} 
                                        className="w-full"
                                    >
                                        <Button className="w-full bg-blue-600 hover:bg-blue-700">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto text-left">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    {role === "admin" && <option value="internship" className="bg-black">Internship</option>}
                                </select>
                            </div>
                            ...
                            {formData.type === 'internship' && role === "admin" && (
                                <div>

...
import { X, Upload, Check, AlertCircle, QrCode, Copy, Smartphone, Building2, CreditCard } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import QRCode from "qrcode";

interface MentorshipPaymentModalProps {
    isOpen: boolean;
    onClose: () => void;
    plan: string;
    amount: number;
...
export function MentorshipPaymentModal({
    isOpen,
    onClose,
    plan,
    amount
}: MentorshipPaymentModalProps) {
    ...
    // UPI Configuration
    const UPI_ID = "singm2698-1@okaxis"; 
    const PAYEE_NAME = "Skill Webory"; 
    ...
    const generateQRCode = async (amount: number) => {
        try {
            const upiUrl = `upi://pay?pa=${UPI_ID}&pn=${encodeURIComponent(PAYEE_NAME)}&am=${amount}&cu=INR&tn=${encodeURIComponent(`Payment for ${plan} Mentorship`)}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                const res = await fetch("/api/mentorship/purchase", {
                    method: "POST",
                ...
                if (res.ok) {
                    toast.success(
                        "🎉 Purchase Successful! Your payment has been received. You'll get access within 1-2 hours after verification.",
                        { duration: 5000 }
                    );
                    onClose();
                } else {
                ...
                >
                    ...
                    <div className="sticky top-0 bg-[#0a0a0a] border-b border-white/10 p-6 flex items-center justify-between z-10">
                        <div>
                            ...
                            <p className="text-gray-400 text-sm mt-1">{plan.charAt(0).toUpperCase() + plan.slice(1)} Mentorship Plan</p>
                        </div>

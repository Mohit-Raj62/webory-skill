...
export default function VideoPlayerPage() {
    const { id: internshipId, videoIndex } = useParams();
    ...
    const [isAccepted, setIsEnrolled] = useState(false);
    ...
    }, [internship, currentIndex, isAccepted, hasMarkedAsWatched]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [resCourse, resProgress] = await Promise.all([
                ...
                fetch(`/api/internships/${internshipId}/progress`)
            ]);
    ...
    const fetchNotes = async () => {
        try {
            const res = await fetch(`/api/internships/${internshipId}/videos/${currentIndex}/notes`);
            if (res.ok) {
    ...
    const fetchFeedback = async () => {
        try {
            const res = await fetch(`/api/internships/${internshipId}/videos/${currentIndex}/feedback`);
            if (res.ok) {
                const data = await res.json();
    ...
    const askDoubt = async () => {
        ...
        try {
            const res = await fetch(`/api/internships/${internshipId}/doubts`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const markAsWatched = async (watchedPercentage: number = 100) => {
        ...
        try {
            ...
            const res = await fetch(`/api/internships/${internshipId}/videos/${currentIndex}/watch`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ 
                    watchedPercentage,
    ...
    const handleLike = async (isLiked: boolean) => {
        ...
        try {
            await fetch(`/api/internships/${internshipId}/videos/${currentIndex}/feedback`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        const fetchUserAndEnrollment = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    // Check enrollment
                    const resEnroll = await fetch("/api/user/applications");
                    if (resEnroll.ok) {
    ...
    const goToNext = () => {
        if (currentIndex < (videosList?.length || 0) - 1) {
            router.push(`/internships/${internshipId}/video/${currentIndex + 1}`);
            setHasMarkedAsWatched(false);
            hasMarkedRef.current = false;
            hasShownOverlayRef.current = false;
                ...
                <div className="flex-1 w-full">
                                ...
                                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center z-[100] animate-in fade-in duration-300">
                                    <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 rounded-2xl border border-gray-700 text-center max-w-md mx-4 shadow-2xl transform scale-100 transition-all">
                                        ...
                                        <div className="flex gap-3 justify-center">
                                            ...
                                            {currentIndex < (videosList?.length || 0) - 1 && (
                                                <Button
                                                    onClick={goToNext}
                    ...
                    <div className="flex items-center justify-between mt-4 bg-gray-900/50 p-2 rounded-xl border border-gray-800 gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <Button
                                onClick={goToNext}
                                disabled={currentIndex >= (videosList?.length || 0) - 1}
                                variant="ghost"
                                className="text-gray-400 hover:text-white hover:bg-gray-800"
                            >
                            ...
                            {showSettings && (
                                <div className="absolute bottom-full right-0 mb-2 w-56 bg-gray-800 rounded-xl border border-gray-700 shadow-xl overflow-hidden z-20 animate-in fade-in slide-in-from-bottom-2">
                                    <div className="p-1 space-y-1">
                                        ...
                                        <button
                                            onClick={() => router.push(`/internships/${internshipId}/doubts`)}
                                            className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-300 hover:bg-gray-700 rounded-lg transition-colors"
                                        >
                        ...
                        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] animate-in fade-in duration-300">
                             <div className="bg-gradient-to-br from-gray-900 to-black p-8 rounded-3xl border border-yellow-500/30 max-w-md w-full mx-4 shadow-2xl relative overflow-hidden">
                                ...
                                <div className="relative z-10 text-center">
                                    ...
                                    <div className="space-y-3">
                                        <Button
                                            onClick={() => router.push(`/internships/${internshipId}?buy=true`)}
                        ...
                        <div className="bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-md rounded-2xl p-5 border border-white/10 sticky top-6 shadow-xl" style={{ maxHeight: 'calc(100vh - 120px)' }}>
                            <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-700">
                                ...
                                <span className="text-xs bg-blue-500/10 text-blue-400 px-2 py-1 rounded-full border border-blue-500/20">
                                    {loading ? "..." : (videosList?.length || 0)} videos
                                </span>
                            </div>
                                ...
                                {loading ? (
                                    ...
                                    [...internship.modules]
                                        ...
                                        .map((module: any, moduleIndex: number) => {
                                            ...
                                            const isModuleSidebarUnlocked = isAccepted || moduleIndex === 0;
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={() => {
                                                                        if (isModuleSidebarUnlocked) {
                                                                            router.push(`/internships/${internshipId}/video/${globalIndex}`);
                                                                            setHasMarkedAsWatched(false);
                                                                        } else {
                                    ...
                                    videosList?.map((video: any, index: number) => {
                                        ...
                                        return (
                                            <button
                                                key={index}
                                                onClick={() => {
                                                    router.push(`/internships/${internshipId}/video/${index}`);
                                                    setHasMarkedAsWatched(false);
                                                }}

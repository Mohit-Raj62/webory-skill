...
export default async function ProfilePage() {
    ...
    const [enrollments, applications, hackathons] = await Promise.all([
        Enrollment.find({ student: user._id })
            .populate("course", "title slug level videos thumbnail description price originalPrice gstPercentage")
            .lean(),
        Application.find({ student: user._id, status: { $ne: "rejected" } })
            .populate("internship", "title slug company location type stipend tags price originalPrice gstPercentage")

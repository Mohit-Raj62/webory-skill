...
export async function GET(req: Request) {
  try {
    ...
    const APPLE_REDIRECT_URI = process.env.APPLE_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/auth/apple/callback`;
    
    if (!APPLE_CLIENT_ID) {
      return NextResponse.json(
        { error: "Apple OAuth is not configured. Please add APPLE_CLIENT_ID to your environment variables." },
        { status: 500 }
      );
    }

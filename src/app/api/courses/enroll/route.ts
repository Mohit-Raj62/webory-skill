  ...
  try {
    ...
    if (promoCode) {
      ...
      if (promo) {
        ...
        const isApplicable =
          promo.applicableTo === "both" || promo.applicableTo === "course";
        const isApplicableToThisCourse =
          promo.applicableIds.length === 0 ||
          promo.applicableIds.includes(courseId);
        const isMinOrderValueMet =
          !promo.minOrderValue || finalPrice >= promo.minOrderValue;

        if (
          !isExpired &&
          !isMaxedOut &&
          isApplicable &&
          isApplicableToThisCourse &&
          isMinOrderValueMet
        ) {

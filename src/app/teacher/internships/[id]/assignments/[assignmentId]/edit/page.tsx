...
export default function EditAssignmentPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch modules
                const internshipRes = await fetch(`/api/internships/${internshipId}?includeUnavailable=true`);
                if (internshipRes.ok) {
                    const internshipData = await internshipRes.json();
                    if (internshipData.internship && internshipData.internship.modules) {
                        setModules(internshipData.internship.modules);
                    }
                }

                // Fetch existing assignment
                const assignmentRes = await fetch(`/api/teacher/internships/${internshipId}/assignments/${assignmentId}`);
                if (assignmentRes.ok) {
                    ...
                    router.push(`/teacher/internships/${internshipId}/assignments`);
                }
            } catch (error) {
        ...
        fetchData();
    }, [internshipId, assignmentId, router]);
    ...
    return (
        <div className="p-8">
            <div className="mb-8">
                <Link href={`/teacher/internships/${internshipId}/assignments`}>
                    <Button variant="ghost" className="mb-4">
                        <ArrowLeft size={20} className="mr-2" />
                        Back to Assignments
                    </Button>
                </Link>
                <h1 className="text-4xl font-bold text-white mb-2">Edit Assignment</h1>
                <p className="text-gray-400">Update the details of your existing assignment</p>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex gap-4 pt-6">
                        ...
                        <Button
                            type="button"
                            variant="outline"
                            onClick={() => router.push(`/teacher/internships/${internshipId}/assignments`)}
                        >
                            Cancel
                        </Button>
                    </div>

...
export default function CreateQuizPage() {
    ...
    useEffect(() => {
        const fetchInternshipModules = async () => {
            try {
                const res = await fetch(`/api/internships/${internshipId}?includeUnavailable=true`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.internship && data.internship.modules) {
                        setModules(data.internship.modules);
                    }
                }
            } catch (error) {
                console.error("Failed to fetch internship modules", error);
            }
        };
        fetchInternshipModules();
    }, [internshipId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/teacher/internships/${internshipId}/quizzes`, {
                method: "POST",
            ...
            if (res.ok) {
                ...
                router.push(`/teacher/internships/${internshipId}/quizzes`);
            } else {
    ...
    return (
        <div className="p-8">
            <div className="mb-8">
                <Link href={`/teacher/internships/${internshipId}/quizzes`}>
                ...
                <p className="text-gray-400">Add quiz/test/exam to your internship</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
                ...
                <div className="flex gap-4">
                    ...
                    <Button
                        type="button"
                        variant="outline"
                        onClick={() => router.push(`/teacher/internships/${internshipId}/quizzes`)}
                    >
                        Cancel
                    </Button>
                </div>
